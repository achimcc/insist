...
file itself). Below is the configuration used to record this repository's
own fixtures against a locally started Alertmanager (`fixtures/constructed-config.toml`),
with every line explained:
...
- `NTFY_BUTTON_TOKEN` — the token embedded in every "Acknowledge" button.
  It can only write to the acknowledgement topic, never the alarm topic.
...
- **No mail.** Mail is sent directly by Alertmanager (or whatever raised
  ...
  the household's plainest alerting channel down with it.
